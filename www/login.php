...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST['username'])) {
        ...
        $username = sql\scripts::getPostVar('username');
    }
    ...
    if (empty($_POST['password'])) {
        ...
        $password = sql\scripts::getPostVar('password');
    }

    if (! empty($_POST['username']) && ! empty($_POST['password'])) {

        $loginStatus = sql\scripts::processLogon($username, $password);
...
<form
	...
	<br> <span><?php echo sql\scripts::getSessVar('errorMsg'); ?></span>
